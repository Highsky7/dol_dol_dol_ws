...
public:
  YawTextMarker() {
    ...
    sub_ = nh.subscribe("/legoloam_yaw_corrected_deg", 1,
                        &YawTextMarker::yawCb, this);
    pub_ = nh.advertise<visualization_msgs::Marker>("legoloam_yaw_corrected_deg_marker", 1);
  }
...
private:
  void yawCb(const std_msgs::Float64& msg) {
    ...
    ss << std::fixed << std::setprecision(precision_)<< "legoloam_yaw" << " " << msg.data << "°";
    m.text = ss.str();
