...
class Judgement:
    def __init__(self):
        ...
        # 속도 플래닝 파라미터
        self.max_throttle = rospy.get_param("~max_throttle", 0.6)  # 최대 스로틀 값 (직진 시 또는 유효 조향각 있을 시)
        self.min_throttle = rospy.get_param("~min_throttle", 0.4)  # 최소 스로틀 값 (최대 조향 시 또는 유효 조향각 없을 시)
