...
class LaneFollowerNode:
    def __init__(self, opt):
        ...
        # 파라미터 파일에 맞는 값으로 설정해야 합니다.
        self.m_per_pixel_y, self.y_offset_m, self.m_per_pixel_x = 0.004015625, 1.83, 0.00278125

        # --- 핵심 이식 1: 추적 상태 변수를 클래스 인스턴스 변수로 선언 ---
    ...
    # --- 핵심 이식 2: process_frame의 로직을 그대로 가져와 클래스에 맞게 수정 ---
    def process_image(self, im0s):
        ...
        # 4. 차선 추적 및 안정화 (self.tracked_lanes 사용)
        ...
        # 5. 주행 경로 생성 및 안정화 (self.tracked_center_path 사용)
        steering_angle_deg = None
        target_center_lane_coeff = None

        if lane_detected_bool:
            center_points = []
            LANE_WIDTH_M, lane_width_pixels = 1.0, 1.0 / self.m_per_pixel_x
        ...
        if self.tracked_center_path['coeff'] is not None:
            ...
            crosstrack_error_m = (self.bev_w / 2 - np.polyval(final_center_coeff, self.bev_h - 1)) * self.m_per_pixel_x
            lookahead_y = self.bev_h - 20
