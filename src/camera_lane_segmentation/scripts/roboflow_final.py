...
# 버전: 1.0 (2025-07-11)
#
# 핵심 로직 (차선, 경로, 동적 전방주시거리): roboflow_with_ROS_dynamicPP.py 기반
# 핵심 기능 (TF, RViz, 장애물 연동): roboflow_final.py 기반
# ===================================================================================
...
import tf2_ros
import tf2_geometry_msgs  # 의존성 유지를 위해 포함
...
class LaneFollowerNode:
    def __init__(self, opt):
        ...
        # 좌표 변환 계수 (bev_params_y_5.npz 기준)
        ...
        # === [로직 유지] dynamicPP.py의 차선 추적 파라미터 ===
        ...
        # === [로직 유지] dynamicPP.py의 Pure Pursuit 및 동적 전방주시거리 파라미터 ===
        self.L = 0.73  # 차량 축거 (Wheelbase) [m]
        self.THROTTLE_MIN = 0.4
        self.THROTTLE_MAX = 0.6
        self.MIN_LOOKAHEAD_DISTANCE = 1.75 # 1.6m
        self.MAX_LOOKAHEAD_DISTANCE = 2.35  # 2.4m
        self.current_throttle = self.THROTTLE_MIN # 초기 throttle 값

        # === [기능 추가] final.py의 장애물(콘) 위치 저장 변수 ===
        self.cones = []

        # === [기능 추가] final.py의 통합 퍼블리셔 ===
        ...
        self.track_sub = rospy.Subscriber("/track", Track, self.track_callback, queue_size=1)

        # === [기능 추가] final.py의 TF 설정 ===
        self.static_br = tf2_ros.StaticTransformBroadcaster()
        self.pub_static_tf()
        
        rospy.loginfo("[Lane Follower] Competition-ready node initialized. Good luck!")

    # === [기능 추가] final.py의 TF 발행 함수 ===
    def pub_static_tf(self):
        ...
        rospy.loginfo(f"[TF] Static transform velodyne -> camera ({tx:.2f}, {ty:.2f}, {tz:.2f}) published.")

    # ────────── 콜백 함수 ────────── #

    # === [로직 유지] dynamicPP.py의 스로틀 콜백 함수 ===
    def throttle_callback(self, msg):
        """스로틀 값을 수신하여 self.current_throttle 변수만 업데이트합니다."""
        self.current_throttle = np.clip(msg.data, self.THROTTLE_MIN, self.THROTTLE_MAX)

    # === [기능 추가] final.py의 장애물 콜백 함수 ===
    def track_callback(self, msg):
        """/track 토픽을 수신하여 콘의 위치를 self.cones 리스트에 저장합니다."""
        self.cones = []
        for cone in msg.cones:
            # 여기서는 모든 타입의 콘을 장애물로 간주합니다.
            # 특정 콘(예: "traffic_cone")만 필터링하려면 아래 주석을 해제하세요.
    ...
    def process_image(self, im0s):
        ...
        # === [로직 유지] dynamicPP.py의 차선 감지, 할당, 스무딩 알고리즘 ===
        ...
        # 4. 차선 추적 및 스무딩 (dynamicPP.py의 Fallback 로직 포함)
        ...
        # === [로직 유지] dynamicPP.py의 중앙 경로 생성 및 Pure Pursuit 제어 ===
        ...
        # === [기능 추가] final.py의 Forced RRT 로직 ===
        x_min, x_max = 1.25, 2.85
        y_min, y_max = -1.145, 1.145
        ...
        # === [기능 추가] final.py의 RViz 시각화 데이터 발행 ===
    ...
    def publish_viz(self, cL, cR, cC, goal):
        ...
        def lane_m(coeff, idx, color):
            ...
            return m
        marr.markers.extend([lane_m(cR, 0, (1, 0, 0)), lane_m(cL, 1, (0, 0, 1))]) # final.py 스타일(우측-빨강, 좌측-파랑)
        ...
        bev_area_marker.pose.orientation.w = 1.0; bev_area_marker.scale.x = 0.03
        ...
        bev_area_marker.points = [
            Point(x=2.85, y=-1.145, z=0.0), Point(x=2.85, y=1.145, z=0.0),
            Point(x=1.25, y=1.145, z=0.0), Point(x=1.25, y=-1.145, z=0.0),
            Point(x=2.85, y=-1.145, z=0.0)
        ]
...
# ────────── 메인 함수 ────────── #
def main():
    rospy.init_node('lane_follower_competition_node', anonymous=True)
    
    parser = argparse.ArgumentParser()
    # dynamicPP.py의 기본값을 사용합니다.
    ...
    parser.add_argument('--param-file', type=str,
                        # default='./bev_params_7.npz')
                        default='./bev_params_y_5.npz')
    ...
    try:
        ...
        rospy.loginfo("Shutting down competition node.")
    finally:
