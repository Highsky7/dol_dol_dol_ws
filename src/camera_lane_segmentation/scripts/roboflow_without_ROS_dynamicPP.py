...
def make_parser():
    ...
    parser.add_argument('--source', type=str,
                        ...
                        default='/home/highsky/Downloads/2025-06-29-145057.mp4',
...
def detect_and_publish(opt, pub_steering, pub_lane_status):
    ...
    # ======================= [핵심 수정 1: 동적 전방주시거리 파라미터 및 상태 관리] =======================
    THROTTLE_MIN = 0.3
    THROTTLE_MAX = 0.5
    MIN_LOOKAHEAD_DISTANCE = 1.4
    MAX_LOOKAHEAD_DISTANCE = 1.8
    
    # 스로틀 상태를 저장할 딕셔너리 (콜백 함수에서 접근 가능하도록)
    throttle_state = {'current': THROTTLE_MIN}

    def throttle_callback(msg):
        """ 'auto_throttle' 토픽을 수신하여 throttle_state 값을 업데이트 """
        throttle_state['current'] = np.clip(msg.data, THROTTLE_MIN, THROTTLE_MAX)

    # 'auto_throttle' 토픽 구독자 생성
    rospy.Subscriber('auto_throttle', Float32, throttle_callback, queue_size=1)
    rospy.loginfo("Subscribing to 'auto_throttle' topic for dynamic lookahead distance.")
    # =================================================================================================

    def process_frame(im0s, tracked_lanes, tracked_center_path, current_throttle):
        ...
        # 5.2 차선 추적 및 연관 로직 (기존과 동일)
        left_lane_tracked, right_lane_tracked = tracked_lanes['left'], tracked_lanes['right']
        ...
        if len(current_detections) == 2: current_left, current_right = current_detections[0], current_detections[1]
            ...
            if dist_to_left < dist_to_right and left_lane_tracked['coeff'] is not None: current_left = detected_lane
                ...
                if detected_lane['x_bottom'] < w_bev / 2: current_left = detected_lane
        ...
        if target_center_lane_coeff is not None:
            if tracked_center_path['coeff'] is None: tracked_center_path['coeff'] = target_center_lane_coeff
            else: tracked_center_path['coeff'] = (SMOOTHING_ALPHA * target_center_lane_coeff + (1 - SMOOTHING_ALPHA) * tracked_center_path['coeff'])
        
        # 5.4 PURE PURSUIT 조향 제어
        if tracked_center_path['coeff'] is not None:
            final_center_coeff = tracked_center_path['coeff']
            L = 0.73  # 차량 축거

            # ======================= [핵심 수정 2: 동적 전방주시거리 계산] =======================
            throttle_range = THROTTLE_MAX - THROTTLE_MIN
            if throttle_range <= 0: normalized_throttle = 0.0
            else: normalized_throttle = (current_throttle - THROTTLE_MIN) / throttle_range
            
            dynamic_lookahead_distance = MIN_LOOKAHEAD_DISTANCE + (MAX_LOOKAHEAD_DISTANCE - MIN_LOOKAHEAD_DISTANCE) * normalized_throttle
            # =================================================================================
            ...
            for y_bev in range(h_bev - 1, -1, -1):
                ...
                if dist >= dynamic_lookahead_distance: # 수정된 부분
            ...
            if goal_point_vehicle is not None:
                ...
                steering_angle_deg = -np.degrees(steering_angle_rad) # 왼쪽이 + 이므로 부호 반전
        ...
        viz_lookahead = MIN_LOOKAHEAD_DISTANCE + (MAX_LOOKAHEAD_DISTANCE - MIN_LOOKAHEAD_DISTANCE) * normalized_throttle
        ...
        cv2.putText(bev_im_for_drawing, f"Throttle: {current_throttle:.2f}", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        # =============================================================================
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # process_frame 호출 시 현재 throttle 값을 전달합니다.
        process_frame(im0s, tracked_lanes, tracked_center_path, throttle_state['current'])
...
def ros_main():
    ...
    # pub_mask는 현재 코드에서 사용되지 않으므로 제거하거나 주석 처리 가능
    ...
    try:
        dataset_to_clean = detect_and_publish(opt, pub_steering, pub_lane_status)
