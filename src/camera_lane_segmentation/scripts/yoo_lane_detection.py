...
def make_parser():
    ...
    parser.add_argument('--param-file', type=str,
                        default='/home/yoo/source/bev_params.npz',
                        help='BEV 파라미터 (src_points, dst_points, warp_w, warp_h)')
    # 추가: 전방주시거리(lookahead, cm 단위)와 휠베이스
    parser.add_argument('--lookahead', type=float, default=150.0,
                        help='Pure Pursuit 전방주시거리 (cm 단위)')
    parser.add_argument('--wheelbase', type=float, default=75.0,
                        help='후륜축 중심으로부터의 휠베이스 (cm 단위)')
    return parser
    ...
    def process_frame(im0s):
        ...
        if coeff is not None:
            ...
            def image_to_vehicle(pt):
                ...
                X_vehicle = (640 - y_img) * 0.475
                Y_vehicle = (x_img - 320) * 0.22680412371 - 2.42 #BEV하단(기존 차량 좌표계 원점)에서 후륜축까지의 거리(meter scale)
                return X_vehicle, Y_vehicle

            # 5. Pure Pursuit: 전방주시거리 (lookahead) 이상 떨어진 목표점 선택
            lookahead = opt.lookahead  # cm 단위
            wheelbase = opt.wheelbase  # cm 단위
            goal_point = None
