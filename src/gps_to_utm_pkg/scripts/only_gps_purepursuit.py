...
class PurePursuit:
    def __init__(self):
        rospy.init_node("only_gps_purepursuit", anonymous=True)
        
        # 파라미터: lookahead distance와 차량 휠베이스 (단위: 미터)
        self.lookahead_distance = rospy.get_param("~lookahead_gps_distance", 3.5)
        self.wheelbase = rospy.get_param("~wheelbase", 0.75)
        
        # ROI Marker를 구독하여 최신 데이터를 저장
        rospy.Subscriber("roi_path_rrt_marker", Marker, self.roi_callback)
        
        # 조향각을 publish할 토픽 (Float32, 단위: degree)
        self.steer_pub = rospy.Publisher("/auto_steer_angle_gps", Float32, queue_size=10)
        
        # Lookahead point Marker를 publish할 토픽
        self.lookahead_pub = rospy.Publisher("lookahead_gps_marker", Marker, queue_size=10)
        
        self.latest_roi_marker = None
        
        # 타이머 콜백: 10Hz (0.1초 간격)
        rospy.Timer(rospy.Duration(0.1), self.timer_callback)
        rospy.loginfo("only_gps_PurePursuit 노드 시작 (lookahead_gps_distance: %.2f m, wheelbase: %.2f m)", 
    ...
    def timer_callback(self, event):
        ...
        rospy.loginfo_throttle(1, "gps_Lookahead: (%.2f, %.2f), dist: %.2f m, α: %.2f deg, steer: %.2f deg",
